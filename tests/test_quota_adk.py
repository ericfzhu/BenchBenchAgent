...
from bba.quota_project import ModelCallQuotaLease
...
class FakeGovernor:
    ...
    def estimate_input_tokens(self, _request):
        return 1234

    def acquire_model_call(self, identity, estimated_input, requested_output):
        self.acquired.append(
            (identity.model, estimated_input, requested_output)
        )
    ...
    def fail(self, _lease, _error):
        pass


class TestQuotaAdkHook(unittest.TestCase):
    def test_each_underlying_model_call_uses_the_granted_output_cap(self):
        identity = next(
        ...
        request = SimpleNamespace(
            config=None,
            model_dump=lambda **_kwargs: {"contents": []},
        )
        asyncio.run(
