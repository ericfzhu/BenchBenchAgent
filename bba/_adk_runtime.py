    ...
    async def consume_events() -> None:
        ...
        while plugin.model_calls < max_llm_calls:
            ...
            if is_finished is None or is_finished():
                break
            if role == "creator":
                continuation = CREATOR_CONTINUATION_PROMPT
            else:
                continuation = SOLVER_CONTINUATION_PROMPT
...
class AdkCreatorBackend(_TraceBackend):
    ...
    def build(
        ...
        message = json.dumps({
            "instruction": CREATOR_INITIAL_INSTRUCTION,
...
class AdkSolverBackend(_TraceBackend):
    ...
    def solve(
        ...
        message = json.dumps({
            "instruction": SOLVER_INITIAL_INSTRUCTION,
            "epoch_digest": manifest.digest,
            "role": "solver",
